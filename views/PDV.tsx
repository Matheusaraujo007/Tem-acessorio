  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 dark:bg-background-dark overflow-hidden font-display">
      <header className="flex items-center justify-between px-8 py-4 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 z-30 shadow-sm">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
             ...
             <div>
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase mt-1">Terminal de Venda / OS</p>
             </div>
          </div>
        ...
        <button onClick={() => window.history.back()} className="px-6 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl font-black text-xs hover:bg-rose-500 hover:text-white transition-all uppercase tracking-widest">Sair</button>
      </header>
          ...
          <div className="flex-1 overflow-y-auto p-6 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 custom-scrollbar">
            {filteredProducts.map(p => (
              <div key={p.id} onClick={() => addToCart(p)} className="bg-white dark:bg-slate-800 p-4 rounded-[2rem] border-2 border-transparent hover:border-primary transition-all cursor-pointer shadow-sm group">
                <div className={`aspect-square rounded-2xl mb-4 overflow-hidden flex items-center justify-center ${p.isService ? 'bg-amber-500/10' : 'bg-slate-100'}`}>
                  {p.isService ? (
                ...
                <div className="flex justify-between items-start">
                   <div>
                      <h4 className="text-xs font-black uppercase truncate max-w-[120px]">{p.name}</h4>
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{p.isService ? 'Mão de Obra' : p.category}</p>
                   </div>
                   {!p.isService && <span className={`text-[10px] font-black ${p.stock <= 5 ? 'text-rose-500' : 'text-emerald-500'}`}>{p.stock}un</span>}
                </div>
        ...
        <aside className="w-[450px] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 flex flex-col shadow-2xl">
          <div className="p-6 space-y-4">
             <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Vendedor / Técnico</label>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
             {cart.length === 0 ? (
               <div className="h-full flex flex-col items-center justify-center opacity-20"><span className="material-symbols-outlined text-6xl">list_alt</span><p className="text-xs font-black uppercase mt-2">Carrinho Vazio</p></div>
             ) : cart.map(item => (
               <div key={item.id} className="flex items-center gap-4 bg-slate-50 dark:bg-slate-800 p-4 rounded-3xl">
                  <div className={`size-12 rounded-xl flex items-center justify-center ${item.isService ? 'bg-amber-500/10 text-amber-500' : 'bg-primary/10 text-primary'}`}>
