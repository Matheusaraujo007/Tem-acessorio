...
const DRE: React.FC = () => {
  ...
  const dreData = useMemo(() => {
    // FILTRAGEM POR UNIDADE PARA DRE PRECISA
    ...
    const rows: DRERow[] = [
      { label: 'RECEITA BRUTA DE VENDAS', value: totalIncome, avPercent: 100.0, trend: 0, isSubtotal: true },
      { label: 'Vendas PDV (Frente de Caixa)', value: sales, avPercent: totalIncome ? (sales / totalIncome) * 100 : 0, trend: 0, indent: true },
      { label: 'Outras Receitas Operacionais', value: otherIncomes, avPercent: totalIncome ? (otherIncomes / totalIncome) * 100 : 0, trend: 0, indent: true },
      { label: '(=) RECEITA OPERACIONAL LÍQUIDA', value: totalIncome, avPercent: 100.0, trend: 0, isSubtotal: true },
      { label: '(-) CUSTO DAS MERCADORIAS VENDIDAS (CMV)', value: -inventoryCosts, avPercent: totalIncome ? (inventoryCosts / totalIncome) * 100 : 0, trend: 0, isNegative: true },
      { label: '(=) MARGEM DE CONTRIBUIÇÃO / LUCRO BRUTO', value: totalIncome - inventoryCosts, avPercent: totalIncome ? ((totalIncome - inventoryCosts) / totalIncome) * 100 : 0, trend: 0, isSubtotal: true },
      { label: '(-) DESPESAS OPERACIONAIS FIXAS', value: -operationalExpenses, avPercent: totalIncome ? (operationalExpenses / totalIncome) * 100 : 0, trend: 0, isNegative: true },
      { label: '(=) RESULTADO LÍQUIDO DO PERÍODO', value: netResult, avPercent: totalIncome ? (netResult / totalIncome) * 100 : 0, trend: 0, isSubtotal: true },
    ];
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        ...
        <div className="flex gap-3 bg-white dark:bg-slate-800 p-2 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
           <button className="flex items-center gap-2 px-6 py-3 bg-slate-100 dark:bg-slate-700 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-primary hover:text-white transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <SummaryCard title="Receita Bruta" value={`R$ ${totalIncome.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`} icon="trending_up" color="text-primary" />
        <SummaryCard title="Rentabilidade" value={`${margin.toFixed(1)}%`} subtext="Margem Líquida" icon="pie_chart" color="text-amber-500" />
        <SummaryCard title="Resultado Final" value={`R$ ${netProfit.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`} icon="account_balance_wallet" color={netProfit >= 0 ? 'text-emerald-500' : 'text-rose-500'} />
      </div>
              ...
              <tr key={i} className={`${row.isSubtotal ? 'bg-slate-50/50 dark:bg-slate-800/30 font-black' : ''} hover:bg-slate-100/50 dark:hover:bg-slate-800/50 transition-all group`}>
                <td className={`px-10 py-6 text-sm uppercase ${row.indent ? 'pl-20 text-slate-500 dark:text-slate-400 font-bold' : 'font-black text-slate-800 dark:text-white'} ${row.isNegative ? 'text-rose-500 !font-black' : ''}`}>
                   <div className="flex items-center gap-3">
                      {row.isSubtotal && <span className="size-2 bg-primary rounded-full"></span>}
                      {row.label}
                   </div>
                </td>
      ...
      <div className="bg-amber-500/10 border-2 border-dashed border-amber-500/20 p-8 rounded-[2.5rem] flex items-center gap-6">
         <div className="size-16 bg-amber-500 text-white rounded-3xl flex items-center justify-center shadow-lg shadow-amber-500/20">
            <span className="material-symbols-outlined text-3xl">lightbulb</span>
         </div>
         <div>
            <h4 className="text-sm font-black text-amber-800 dark:text-amber-400 uppercase tracking-widest">Dica de Gestão</h4>
            <p className="text-xs font-bold text-amber-700/70 dark:text-amber-400/70 uppercase leading-relaxed mt-1">
               O CMV (Custo de Mercadoria) é calculado automaticamente baseando-se nas entradas bipadas no módulo de Inventário. <br/>
               Certifique-se de que o Preço de Custo esteja correto no cadastro de cada produto para uma DRE fiel à realidade.
            </p>
         </div>
      </div>
    </div>
